...
for step in range(0,steps+100,100):
	...
	ax = av_df02.plot(xlim=(100,150), ylim=(0,50),style=['rs-','b^-','yo-','k*:'], title='%i K, $ε_e$ = %.3f'%(temp,estrain), alpha=0.5,mark_right=False)
	#ax = av_df02.plot(secondary_y= ['mean'], xlim=(100,150), ylim=(0,50),style=['rs-','b^-','yo-','k*:'], title='%i K, $ε_e$ = %.3f'%(temp,estrain), alpha=0.5,mark_right=False)
	...
	ax.legend_.remove()
	ax.legend(labels = ['Cu','O','Zr'],loc='upper right',fontsize=legendfontsize)
	...
	fig = plt.figure(figsize=figuresize)
	ax = cn_df02.plot(xlim=(100,150), ylim=(5,25),style=['rs-','b^-','yo-','k*:'], title='%i K, $ε_e$ = %.3f'%(temp,estrain), alpha=0.5,mark_right=False)
